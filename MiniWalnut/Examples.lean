...
def tmsquarelengths : DFA_extended (List B2) Nat :=
  ...
  -- T[j+i] = T[j+n+i]
  let Ta_equals_Tb_and_a_ji := minimization (crossproduct t_a_equals_t_b (binary_ops.logical_op l_ops.and) a_equals_j_p_i)
  let Ea_Ta_equals_Tb_and_a_ji := minimization (quant Ta_equals_Tb_and_a_ji 'a' quant_op.exists)
  ...
  let Ta_equals_Tb_and_a_ji_and_b_jni := minimization (crossproduct Eab_Ta_equals_Tb_and_a_ji_and_b_jc (binary_ops.logical_op l_ops.and) c_equals_n_p_i)
  let Eabc_Ta_equals_Tb_and_a_ji_and_b_jni := minimization (quant Ta_equals_Tb_and_a_ji_and_b_jni 'c' quant_op.exists)
  -- i < n & T[j+i] = T[j+n+i]
  let i_lt_n := build_less_than_DFA 'i' 'n'
  let i_lt_n_impl_Tji_equals_Tjni := minimization (crossproduct i_lt_n (binary_ops.logical_op l_ops.impl) Eabc_Ta_equals_Tb_and_a_ji_and_b_jni)
  -- Ej Ai (i < n) => T[j+i] = T[j+n+i]
  let Ai_i_lt_n_and_Tji_equals_Tjni := minimization (quant i_lt_n_impl_Tji_equals_Tjni 'i' quant_op.for_all)
  let Ej_Ai_i_lt_n_and_Tji_equals_Tjni := minimization (quant Ai_i_lt_n_and_Tji_equals_Tjni 'j' quant_op.exists)
  Ej_Ai_i_lt_n_and_Tji_equals_Tjni
...
def paltm : DFA_extended (List B2) Nat :=
  ...
  let j_lt_n_impl_Tij_Tin_m_j1 := minimization (crossproduct j_lt_n (binary_ops.logical_op l_ops.impl) Eb_Tij_equals_Tb_and_b_in_minus_j1)
  let Ai_i_lt_n_and_Tij_Tin_m_j1 := minimization (quant j_lt_n_impl_Tij_Tin_m_j1 'j' quant_op.for_all)
  let Ej_Ai_i_lt_n_and_Tij_Tin_m_j1 := minimization (quant Ai_i_lt_n_and_Tij_Tin_m_j1 'i' quant_op.exists)
  Ej_Ai_i_lt_n_and_Tij_Tin_m_j1
...
def order_of_squares_in_th_word : DFA_extended (List B2) Nat :=
  ...
  let a_lt_n_and_a_eq_0 := minimization (crossproduct a_lt_n (binary_ops.logical_op l_ops.and) a_eq_0)
  let n_geq_1 :=  minimization (quant (a_lt_n_and_a_eq_0) 'a' quant_op.exists)
  -- Ei n > 0 & (Ak k < n => T [i + k] = T [i + n + k])
  let squares_in_th_word :=  minimization (crossproduct n_geq_1 (binary_ops.logical_op l_ops.and) Ak_k_lt_n_impl_t_ik_equals_t_ink)
...
def thue_morse_does_not_have_overlaps : DFA_extended (List B2) Nat :=
  ...
  let n_gt_0_and_Ak_k_lteq_n_impl_t_ik_eq_t_ink :=  minimization (crossproduct n_geq_1 (binary_ops.logical_op l_ops.and) Ak_k_lteq_n_impl_t_ik_equals_t_ink)
...
def tmunb : DFA_extended (List B2) Nat :=
  ...
  let Eab_t_a_nequals_t_b_and_a_it_and_b_cj := minimization (quant t_a_nequals_t_b_and_a_it_and_b_cj 'b' quant_op.exists)
  let t_a_nequals_t_b_and_a_it_and_b_ind := minimization (crossproduct Eab_t_a_nequals_t_b_and_a_it_and_b_cj (binary_ops.logical_op l_ops.and) c_equals_i_p_d)
  let Eabc_t_a_equals_t_b_and_a_it_and_b_ind := minimization (quant t_a_nequals_t_b_and_a_it_and_b_ind 'c' quant_op.exists)
  let t_a_nequals_t_b_and_a_ik_and_b_intj := minimization (crossproduct Eabc_t_a_equals_t_b_and_a_it_and_b_ind (binary_ops.logical_op l_ops.and) d_equals_n_p_t)
  let Tik_neq_Tintj := minimization (quant t_a_nequals_t_b_and_a_ik_and_b_intj 'd' quant_op.exists)
  -- Et (t < j) & T[i+t] != T[(i+n+t)-j]
  ...
  -- Ei Aj (j>=1 & 2*j<=n) => Et (t < j) & T[i+t] != T[(i+n+t)-j]
